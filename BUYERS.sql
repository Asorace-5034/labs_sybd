...
GO
CREATE FUNCTION SATURDAY (@PurchaseId INT)
RETURNS NVARCHAR(3)
AS
BEGIN
    DECLARE @Date INT
    ...
    SELECT @Date = DATEPART(WEEKDAY,DATETIMES) FROM Purchase  WHERE id = @PurchaseId
...
END;
GO
SELECT dbo.SATURDAY(id) AS is_saturday FROM Purchase;